...
const CardDrawer = ({ drawerOpen, toggleCartDrawer }) => {
    ...
    return (
        <>
            {/* Overlay */}
            {drawerOpen && (
                <div className='fixed inset-0 bg-black bg-opacity-40 z-40' onClick={toggleCartDrawer}>

                </div>
            )}
            {/* Drawer */}
            <div className={`fixed top-0 right-0 w-3/4 sm:w-1/2 md:w-1/3 h-full bg-white shadow-lg 
            transform transition-transform duration-300 flex flex-col z-50 ${drawerOpen ? "translate-x-0" : "translate-x-full"}`}>
                ...
                <div className='flex-grow p-4 overflow-y-auto'>
                    ...
                    {cart && cart?.products?.length > 0 ? (
                        <CartContent className=" " cart={cart} userId={userId} guestId={guestId} />
                    ) : (
